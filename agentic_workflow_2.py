...
# End-to-end ReAct agent to ask subquestions related to the query 
...
You DON'T NEED to gather information IF :
- the user's query can be answered based on the history of the conversation.
- the user's query is not about the aerospace project.
If you don't need to gather information, output that you are done.
...
INSTRUCTIONS FOR WHEN INTERACTING WITH THE LLMS :
    1. Ask the LLMs for clarification or details whenever needed.
    2. Adapt the depth and number of your questions based on the level of detail requested by the user.
...
You may need to ask multiple rounds of questions to gather enough details to answer the query comprehensively."""
    ...
    agent_messages = serialize_messages(messages)
    log_text = log_step("AskSubquestionsWithReactAgent", {"messages": agent_messages}, "", retrieval_statistics = retrieval_statistics)
    log.append(log_text)
    agent_tool_calls_only = agent_messages[2+len(history):-1]
    direct = False
    if agent_tool_calls_only == [] :
        direct = True
    return({"final_answer": final_answer, "log":log, "retrieval_statistics":retrieval_statistics, "agent_messages":agent_tool_calls_only, "direct":direct})

# Produce the final answer based on the informations collected by the ReAct agent
...
# Produce the final answer based on the history / LLM knowledge 
def direct_answer(state):
    ...
    Ground your answer as much as possible on the previous chat history and try to keep the same tone, formatting, way of including sources as the previous answers. Do NOT hallucinate nor invent informations.
    """
    messages = [
        {
            "role":"system",
...
# Get the pages referenced in the final answer
...
You may be given potential references to help you.
    """ 
    messages = [{"role":"system", "content": system_prompt}]
    prompt = f"""
...
graph.add_conditional_edges(
    "AskSubquestionsWithReactAgent",
    lambda state: state["direct"],
    {
        True: "DirectAnswer",
        False: "ProduceFinalAnswer",
    },
)
...
# Entry point
graph.set_entry_point("AskSubquestionsWithReactAgent")
